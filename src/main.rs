...
fn main() {
    ...
    let (n, _e, d) = (4119524081, 65537, 3373923173);
...
fn pow_loop(base: u64, exp: u64, n: u64) -> u64 {
    ...
    if exp % 2 == 1 {
        pow_loop(base*base % n, (exp-1)/2, n) * base % n
    } else {
    pow_loop(base*base % n, exp/2, n) % n
    }
}
...
fn receive(d: u64, n: u64, message_encrypted:&str) {
    ...
    for chunk in message_encrypted.chars().collect::<Vec<_>>().chunks(8) {
        ...
        let chunk_u64 = u64::from_str_radix(&chunk_str, 16).unwrap();
...
fn uint_to_str(message_uint: &[u32]) -> String {
